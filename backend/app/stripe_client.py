...
class StripeClient:
    ...
    def __init__(self):
        """Stripe クライアントの初期化"""
        if not settings.stripe_secret_key or settings.stripe_secret_key.startswith("placeholder"):
    ...
    async def create_checkout_session(
        ...
        try:
            session = stripe.checkout.Session.create(
                ...
                payment_method_configuration=settings.stripe_payment_method_config if hasattr(settings, 'stripe_payment_method_config') else None
            )
    ...
    async def create_billing_portal_session(
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Billing portal creation failed: {str(e)}")
